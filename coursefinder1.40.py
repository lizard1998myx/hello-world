...
def searcherio():
    ...
    if start == -1:
        ...
        searcher(start, int(input("输入结束值："))+1)
    input("\n任意键以退出")
...
def tester():
    ...
    print("测试开始，请等待测试结果……")
    i = 0
    for Course_code in list(range(163600, 163610)) + list(range(165400, 165410)) + list(range(158400, 158410)):
...
def searcher(start, end):
    ...
    recorder_init(filename)
    for Course_code in range(start,end):

        # 进度条显示器
        percent = 1.0 * (Course_code - start + 1) / (end - start)
        sys.stdout.write("\r进度 {0}{1}{2}".format("|" * int(percent // 0.05),
                                                   '%.2f%%' % (percent * 100),
                                                   " (" + str(Course_code) + ")"))
        sys.stdout.flush()
...
def infofinder(Course_code):
    ...
    if Course_name == "": #如果没东西
        ...
        try:
            ...
            bs0bj_term = BeautifulSoup(html_term, "html.parser") #这是用来干啥的？
